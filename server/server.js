...
app.get('/api/download-the-app',(req,res)=>{
  console.log('here now')
  res.download(path.join(__dirname,'app-release.apk'))
})
...
app.post('/register', (req,res)=>{
  ...
  pool.query(`SELECT * FROM users where email = $1`,[email],(err,results)=>{
    ...
    if(results.rows.length > 0){
      ...
      VALUES ('${email.toLowerCase()}','${name}','${password}','user-img','false')`,(err,results)=>{
        if(err) throw err
...
//CREATE AND GET SESSION ON DB FOR WEB
app.post('/create-session-id',(req,res)=>{
  pool.query(`UPDATE users SET session_id = '${req.body.session_Id}' WHERE email = '${req.body.email}'`)
  console.log(req.body.session_Id,' stored')
  res.json({msg:'session stored'})
})

app.get('/get-session-id',(req,res)=>{
  pool.query(`SELECT * from users WHERE session_id = '${req.query.n_s_id}'`,(err,results)=>{
    if(err) throw err
    if(results.rows.length > 0){
      res.json({session:true,details:results.rows})
      console.log(results.rows)
...
app.post('/update-profile-web', async (req,res)=>{
  const { id,name,email,password } =req.body
  pool.query(`UPDATE users SET fullname = '${name}',password='${password}', email = '${email.toLowerCase()}' WHERE id = ${id}`,(err,results)=>{
    if(err){
      res.json({msg:"An error occurred, the email might be registered already.",success:false})
    }else{
